...
public class Test 
{
    private static final String COMPANY_COUNTRY_CODE = "+91";
    public static void main(String[] args) 
    {
        ...
        System.out.println("The Employee Details in Descending Order :--");
        arrangeEmployeeBySalary(employees);
        System.out.println("The Employeees by Job Position is Manager :- ");
        getEmployeesByJobPosition(employees, "manager");
        int count = foreignEmployeeCount(employees);
        System.out.println("Number of foreign employees: " + count);
        System.out.println("The Employees Whose Salary is in a range 150000 INR to 300000 INR : ");
    ...
    public static void arrangeEmployeeBySalary(Employee e[]) 
    {
        ...
        Arrays.sort(e, (e1, e2) -> Double.compare(e2.getSalary(), e1.getSalary()));
    ...
    public static void getEmployeesByHireDate(Employee e[] , Date D1, Date D2) 
    {
        ...
        for (Employee employee : e) 
        {
            if (employee.getHireDate().isBetween(D1, D2)) 
            {
    ...
    public static int foreignEmployeeCount(Employee e[] ) 
    {
        int count = 0;
        for (Employee employee : e) {
            String countryCode = employee.getContactNumber().substring(0, 3); // Extracting the country code from the contactNumber
            if (!countryCode.equals(COMPANY_COUNTRY_CODE)) 
            {
                count++;
            }
        }
    ...
    public static void getEmployeesBySalary(Employee e[], double s1, double s2) 
    {
        for (int i = 0; i < e.length; i++) 
        {
            if (e[i].getSalary() >= s1 && e[i].getSalary() <= s2) 
            {
                ...
                System.out.println("Hire date: " + e[i].getHireDate());
                System.out.println("Job position: " + e[i].getJobPosition());
                System.out.println("Contact number: " + e[i].getContactNumber());
